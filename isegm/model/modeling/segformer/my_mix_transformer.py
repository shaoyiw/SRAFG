...
from isegm.model.modeling.segformer.models import Block, OverlapPatchEmbed, AttnWeightBlock, BlockCA, GuidanceEmbed
import math
...
class MixVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.patch_embed_side1 = OverlapPatchEmbed(img_size=img_size, patch_size=7, stride=4, in_chans=2,
                                                   embed_dim=embed_dims[0])
        self.guidance_embed = GuidanceEmbed(in_chans=3, embed_dim=embed_dims[1])

        self.block_fusion = nn.ModuleList([BlockCA(
            dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0., norm_layer=norm_layer,
            sr_ratio=sr_ratios[1])
            for _ in range(4)])
        self.norm_fusion = norm_layer(embed_dims[1])
    ...
    def forward_features(self, x, additional_feature, guidance_signal, points, gate):
        B = x.shape[0]
        ...
        # fusion stage
        guidance_feature = self.guidance_embed(guidance_signal)  # -> (B, L, C)
        for blk in self.block_fusion:
            # guidance_feature 是 KV，x_s2 是 Q
            x_s2 = blk(x_s2, guidance_feature, H, W)
        x_fused = x_s2_orig + x_s2 * gate
        x = self.norm_fusion(x_fused)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
    ...
    def forward(self, x, additional_feature, guidance_signal, *args): # 增加 guidance_signal 参数
        x = self.forward_features(x, additional_feature, guidance_signal, *args)
        return x
