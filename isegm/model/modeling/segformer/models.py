...
class BlockCA(nn.Module):
    ...
    def forward(self, x, x_side, H, W):
        x_side = x_side + self.drop_path(self.attn1(self.norm1(x_side), self.norm1(x_side), H, W))

        x_cross = self.attn2(self.norm_side(x), self.norm2(x_side), H, W)

        gate_input = torch.cat([x, x_side], dim=-1)
        gate_value = self.gate(gate_input) * 2.0
        x_fused = x + gate_value * self.drop_path(x_cross)

        x_fused = x_fused + self.drop_path(self.mlp(self.norm3(x_fused), H, W))
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W, w=None, use_weight=False):
        if use_weight and w is not None:  # Ensure weight is not None
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        # The following is not used if the image size is dynamic
...
class GuidanceEmbed(nn.Module):
    def __init__(self, in_chans=3, embed_dim=128): # in_chans = corrected_mask (1) + click_map (2)
        super().__init__()
        self.proj = nn.Sequential(
            nn.Conv2d(in_chans, 64, kernel_size=3, stride=2, padding=1), # -> H/2, W/2
            nn.BatchNorm2d(64),
            nn.GELU(),
            nn.Conv2d(64, embed_dim, kernel_size=3, stride=2, padding=1), # -> H/4, W/4
            nn.BatchNorm2d(embed_dim),
            nn.GELU(),
            nn.Conv2d(embed_dim, embed_dim, kernel_size=3, stride=2, padding=1), # -> H/8, W/8
            nn.BatchNorm2d(embed_dim)
        )
