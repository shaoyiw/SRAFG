from .my_mix_transformer_v2 import mit_b0, mit_b1, mit_b2, mit_b3, mit_b4, mit_b5
...
class SegFormer(nn.Module):
    ...
    def forward(self, x, additional_features, guidance_signal, *args):
        with ExitStack() as stack:
            if self.inference_mode:
                stack.enter_context(torch.no_grad())

            # 将 guidance_signal 传入 backbone
            outs = self.backbone(x, additional_features, guidance_signal, *args)
            pred = self.decode_head(outs['outs'])
